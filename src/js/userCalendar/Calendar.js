...
const Calendar = ({currentYear,currentMonth}) => {
  ...
  // 해당 달의 마지막 날
  const currentMonthLastDate = new Date(nextMonth - 1).getDate();
  // 지난 달의 마지막 날
  const lastMonthLastDate = new Date(currentMonthFirstDate - 1).getDate();
  const weeksNm = ["일","월","화","수","목","금","토"];
  // 해당 달의 1일의 요일
  const firstDay = currentMonthFirstDate.getDay();

  // 달력 상 첫번째 날
  let calendarFirstDay = lastMonthLastDate - firstDay + 1;

  // 달력에서 보일 최대 일 수 = 42
  const CALENDARNUMBER = 42;
    ...
    <div className="w-full h-[50rem]">
        <table className="w-full shadow-2xl">
            <thead className="h-[10%] text-white bg-[#06B6D4]">
                <tr>
                    {weeksNm.map((item,idx) => <th className="border-black border" key={`key${idx}`}>{item}</th>)}
                </tr>
            </thead>
            <tbody>
                {
                      ...
                      {   
                        ...
                        <tr key={`key${item}`}>
                            {dates.slice(7*item,7*item+7).map((d,idx) => {
                                ...
                                return <DayComponent key={`key${idx}`} innerWidth={innerWidth} currentYear={currentYear} currentMonth={currentMonth} isLastMonth={isLastMonth} isNextMonth={isNextMonth}  d={d}/>
                            })}
                        </tr>
                      )})
                }
