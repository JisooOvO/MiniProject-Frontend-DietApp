const Bar = ({serveMn, originalServeMn, nutr, color, isKcal=false}) => {
  const w = serveMn / originalServeMn;
  const absolNutr = Math.round(nutr * w);
  let BarWidth = absolNutr;
  let unit;
  ...
  return (
    <div className="border h-[70%] w-[90%] relative rounded-md overflow-hidden">
        <div className="absolute z-10 right-2 text-[80%] top-[50%] translate-y-[-50%]">{absolNutr}{unit}</div>
